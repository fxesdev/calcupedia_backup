...
<li>The 8-key test and the debug menu make a return. The 8-key test has different keys displayed and actually refers to the CW models' keymap. The debug menu replaced the key test with <span class="espl">Key1 test</span> and <span class="espl">Key2 test</span>, which allows testing of only the first 27 keys (excluding <span class="k_cw">W</span>) or the last 20 keys.</li>
...
<li><span class="cwcwd">Bt OK</span>: It is assumed that <span class="cwcwd">OK</span> stands for Battery, so this may be a battery model check.</li>
